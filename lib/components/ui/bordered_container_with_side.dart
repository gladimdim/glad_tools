...
class BorderedContainerWithSides extends StatelessWidget {
  ...
  final Color color;
  final Widget child;
  final List<AxisDirection> borderDirections;

  const BorderedContainerWithSides({
    Key? key,
    this.width = 3.0,
    this.color = Colors.black,
    required this.child,
    this.borderDirections = const [AxisDirection.up],
  }) : super(key: key);
  ...
  Map<AxisDirection, BorderSide> get _sidesForDirection {
    return {
      ...
      AxisDirection.down: borderDirections.contains(AxisDirection.down)
          ? _side
          : BorderSide.none,
      AxisDirection.left: borderDirections.contains(AxisDirection.left)
          ? _side
          : BorderSide.none,
      AxisDirection.right: borderDirections.contains(AxisDirection.right)
          ? _side
          : BorderSide.none,
    };
  }
